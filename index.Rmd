...
# cases
model1 = glm(new_cases ~ days_tabligh + days_mco + recover, family = "quasipoisson",  data = covid_my_full)
model_r = glm(recover ~ days_first, "quasipoisson", data = covid_my_full); summary(model_r)
recover_ = round(predict(model_r, list(days_first = days_first_now + 1:7), "response"))
data_week = data.frame(date = date_now + 1:7, new_cases = rep(NA, 7), total_cases = rep(NA, 7),
                       days_tabligh = days_tabligh_now + 1:7, days_mco = days_mco_now + 1:7, recover = recover_)
data_plot = subset(covid_my_full, select = c(date, new_cases, total_cases, days_tabligh, days_mco, recover))
...
### New cases, hover over the graph for details.
```{r}
# daily
...
### Total cases, hover over the graph for details.
```{r}
# cumulative
...
### **Significant factors:** 1. Days since Tabligh gathering (`r paste0("Rate ratio: ", round(model1_rr[2, "estimate"],2), " [95% CI: ", round(model1_rr[2, "conf.low"],2), ", ", round(model1_rr[2, "conf.high"],2), "]")` per day), 2. Days since movement control order (`r paste0("Rate ratio: ", round(model1_rr[3, "estimate"],2), " [95% CI: ", round(model1_rr[3, "conf.low"],2), ", ", round(model1_rr[3, "conf.high"],2), "]")` per day), 3. Number of cases recovered (`r paste0("Rate ratio: ", round(model1_rr[4, "estimate"],2), " [95% CI: ", round(model1_rr[4, "conf.low"],2), ", ", round(model1_rr[4, "conf.high"],2), "]")` per case recovered) based on a quasipoisson regression model. {.value-box}
```{r}
valueBox(
  value = "",
)
...
### New deaths, hover over the graph for details.
```{r}
# daily
...
### Total deaths, hover over the graph for details.
```{r}
# cumulative
...
plot7 = ggplot(data = data_plots) +
  ...
  scale_x_date(date_breaks = "3 day", date_labels = "%d/%m/%y") +
  scale_fill_discrete(name="State") +
  labs(x="Date", y="New deaths") + 
  theme_light()
plot7
