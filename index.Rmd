...
icu_my = icu_state %>% group_by(date) %>%
  ...
  mutate(icu_util_percent = icu_util*100, vent_util_percent = vent_util*100) %>%
  add_column(state = "Malaysia", .after = "date")
icu_my$state = as.factor(icu_my$state)

# -- State --
icu_state_short = icu_state %>% 
  select(date, state, beds_icu_covid, vent, icu_covid, vent_covid) %>%
  mutate(icu_util = icu_covid / beds_icu_covid,
                                 vent_util = vent_covid / vent)  %>%
  mutate(icu_util_percent = icu_util*100, vent_util_percent = vent_util*100)
...
data_plot_icu_state = icu_state_short
data_plot_icu_state_my = full_join(data_plot_icu, data_plot_icu_state) %>% arrange(date)
# includes Malaysia for comparison
...
# ICU by State {data-orientation=columns}
...
Data by `r paste0(format(sel_date_icu, "%d %B %Y"), ", 11:59PM")`.
...
### ICU bed utilization % for COVID-19, hover/tap on the graph for details.
```{r}
# today

plot_icu_state = ggplot(subset(data_plot_icu_state_my, date == sel_date_icu)) +
  geom_col(aes(y = state, x = icu_util_percent, fill = state), alpha = 0.7) +
  scale_y_discrete(limits = rev) +
  geom_vline(xintercept = 50, linetype = "dotted") + 
  geom_vline(xintercept = 100, linetype = "dotted") + 
  scale_fill_discrete(name = "State") +
  labs(y = "State", x = "ICU Utilization %", title = paste0("ICU Utilization % on ", format(sel_date_icu, "%d %B %Y"))) +
  theme_classic()
...
### Ventilator utilization % for COVID-19, hover/tap on the graph for details.
```{r}
# today

plot_vent_state = ggplot(subset(data_plot_icu_state_my, date == sel_date_icu)) +
  geom_col(aes(y = state, x = vent_util_percent, fill = state), alpha = 0.7) +
  scale_y_discrete(limits = rev) +
  geom_vline(xintercept = 50, linetype = "dotted") + 
  geom_vline(xintercept = 100, linetype = "dotted") + 
  scale_fill_discrete(name = "State") +
  labs(y = "State", x = "Ventilator Utilization %", title = paste0("Ventilator Utilization % on ", format(sel_date_icu, "%d %B %Y"))) +
  theme_classic()

ggplotly(plot_vent_state)
```
